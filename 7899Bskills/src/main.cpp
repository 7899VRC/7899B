...
void ladybrownAuto()
{
  ...
  while (true)
  {
    ...
    else if (autoposition == 2)
    {
      ...
      Ladybrown.spinTo(0, deg, true);
  }
}

void loweststake()
{
  if (!(ladybrownposition == 4) && ladybrownposition <= 0 || ladybrownposition == 6)
  {
  ...
  else if (!(ladybrownposition == 6))
  {
    Ladybrown.spinTo(360, deg);
    ladybrownposition = 6;
  }
}
...
void ladybrownmacro()
{
  ...
  {
    Ladybrown.spinTo(34.5, degrees, true);
    ladybrownposition = 1;
  }

  else
  {
    Ladybrown.spinTo(260, degrees, true);
    ladybrownposition = 2;
  }
}
int stuckcount = 0;
...
void intakecontrol()
{
  ...
  while (true)
  {
    ...
    // identify jam
    if (justStartingCnt == 0 && isRollerSpinningForward && fabs(roller.velocity(pct)) <= 5)
    {
      stuckcount += 1;
...
void gyroTurnF(float target, double accuracy = 0.3, float b = 2.4)
{
  float heading = 0.0; // initialize a variable for heading
  double error = target - heading;
  double ki = 0.25;
...
void arcturnLC(int r, float arcdeg, int timeLimit, float max_drift = 0.15)
{
  float heading = Gyro.rotation();
...
void autonomous(void)
{
  ...
  vex::thread aliance(loweststake);
  isRollerSpinningForward = false;
  wait(450, msec);
  inchDrive(-7, 700);
  gyroTurnF(-90);
...
void usercontrol(void)
{
  ...
  while (true)
  {
    ...
    // }
    double t = 4.2;
    double curvedTurn = pow(M_E, (t * (fabs(rstick) - 100.0) / 1000.0)) * rstick;
    std::cout << curvedTurn << std::endl;
    curvedTurn = rstick;
    drive((lstick) + (curvedTurn), (lstick) - (curvedTurn), 20);
    wait(20, msec);
  }
}
